...
export class DeploymentsService {
  ...
  async create(userId: string, dto: CreateDeploymentDto) {
    ...
    const matchingAccount = gitAccounts.find(acc => acc.username === project.gitRepoOwner) || gitAccounts[0];
    ...
    for (const v of projectEnvVars) {
      ...
      try {
        ...
        this.logger.warn(`Failed to decrypt var ${v.key} for project ${projectId}`);
      }
    }
  ...
  async findAllByProject(userId: string, projectId: string) {
    // Check access first
    const project = await this.prisma.project.count({ where: { id: projectId, userId } });
    if (!project) throw new NotFoundException('Project not found');
